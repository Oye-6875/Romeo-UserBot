...
from Romeo import ids as list_users
...
def paginate_help(page_number, loaded_modules, prefix):
    ...
    if len(pairs) > number_of_rows:
        pairs = pairs[
            ...
            (
                InlineKeyboardButton(
                    text="🔥𝐁𝐀𝐂𝐊", callback_data=f"{prefix}_prev({modulo_page})"
                ),
                InlineKeyboardButton(text="•❌•", callback_data="close_help"),
                InlineKeyboardButton(
                    text="𝐍𝐄𝐗𝐓🔥", callback_data=f"{prefix}_next({modulo_page})"
                ),
            )
        ]
    return pairs
